...
const MapComponent = ({
    ...
    useEffect(() => {
        if (isMissionBuilding && isPlacingMarker && mapRef.current) {
            // Очистка существующих маркеров маршрута
            markersRef.current.forEach((marker) => marker.remove());
            markersRef.current = [];

            // Добавляем новые маркеры маршрута
    ...
    }, [routePoints, isMissionBuilding, isPlacingMarker]);
